...
> **Notas de implementação (fase 5, Checkout — entregue).** Cinco pontos que o spec deixou em aberto e foram decididos na implementação:
> - **O desconto Pix com sinal ligado.** O spec proíbe aplicá-lo *sobre o sinal* e não diz o que fazer com ele no resto. Decisão: com sinal ligado, `DESCONTO_PIX` **não é emitido** — zerado na origem, pelo mesmo mecanismo que já o zerava quando a forma de pagamento não é "os dois". O campo **não** é escondido: fica na tela, desabilitado, com aviso âmbar dizendo por que não vale, porque esconder faria os 10% já configurados sumirem sem explicação. A alternativa descartada era abater o desconto no **saldo** — seria inventar regra que o spec não autoriza, e prometer agora um abatimento sobre um pagamento que acontece depois, fora do carrinho. A leitura adotada de "o sinal sai do total já com desconto" é **desconto do cupom**: é o desconto citado na frase anterior do spec, e é a única leitura que não entra em contradição com "o desconto Pix não se aplica sobre o sinal".
> - **Onde a recusa de "sinal maior que o total" mora.** Em dois lugares, com critérios diferentes. Sinal fixo acima do **maior pedido que o carrinho consegue montar** (já contando o teto da quantidade) é recusado **na geração**, porque nenhum pedido seria pagável e não há valor certo a assumir. Sinal maior que o total **do pedido em curso** é recusado **no carrinho**, com linha vermelha, `actions.reject()` no PayPal e Pix sem payload. Sinal fixo acima do pedido *mínimo* não é recusado na geração de propósito: pode ser piso de venda intencional. Recusa, nunca aparo — aparar cobraria um valor que o operador não configurou.
> - **O seletor de quantidade vai de 0 até o teto**, e a quantidade zero tem **um corte só** (`produtoAtivo`): é por ele que o item sai da conta, da descrição do PayPal, da mensagem do WhatsApp e do resumo. As quantidades do produto e do opcional são **independentes** — dois ensaios com um álbum somam dois ensaios e um álbum.
> - **O resumo (R3) nasce desligado**, como a cobrança de sinal, porque a regressão exige que o bloco com os recursos desligados saia byte a byte igual ao anterior — e um resumo emitido sempre quebraria isso. Com ele ligado, a lista de itens da mensagem do WhatsApp passa a vir do **mesmo enumerador** do resumo: dois laços separados poderiam divergir na regra da quantidade zero e as duas listas discordariam caladas.
> - **A prévia executa as recusas do gerador, não só o gerador** (rodada de correção 1). As guardas viraram `uRecusa(cfg)`, função pura, usada pela geração **e** pela prévia; com configuração recusada a prévia mostra o **quadro da recusa com o motivo**, nunca o carrinho e nunca o último bloco válido rotulado como desatualizado. E o shim do PayPal passou a **exercitar** o `createOrder` e o `onClick` do próprio bloco, mostrando o valor que iria na ordem e a decisão do clique — o valor do sinal e as recusas do lado PayPal ficam verificáveis sem baixar o SDK. O que a prévia continua não provando está declarado na legenda: o que depende da resposta do PayPal (janela de pagamento, captura e as mensagens que vêm dela).
> - **A prévia foi convertida** para executar o bloco gerado num iframe, como Contagem, Slideshow e Leads. Ela era uma segunda implementação e já havia divergido uma vez; com três recursos novos, seriam três lugares a mais onde a divergência pode nascer. O ambiente da prévia troca duas coisas, com o bloco entregue intacto: storage em memória (com aviso visível se a troca não pegar) e o **SDK do PayPal substituído pelo desenho** que a prévia antiga já usava — carregá-lo mandaria o Client ID para fora a cada remontagem e mostraria erro de pagamento falso com o campo ainda vazio. O script do QR continua sendo baixado, então o QR da prévia é o do payload real.
