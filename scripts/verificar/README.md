...
| `duplicar-itens.mjs` | O BOTAO DE DUPLICAR da aba Agendamento por pacote, e o que ele nunca pode sobrescrever. Sete casos: duplicar um pacote COM OPCIONAIS (o formulario vem completo, em modo criar, e salvar cria um SEGUNDO pacote -- o original e comparado campo a campo antes e depois), duplicar ENQUANTO EDITA outro pacote (o que estava em edicao fica intacto), duplicar e RECARREGAR sem salvar (o estado persistido devolve a copia ainda em modo criar), duplicar um OPCIONAL (a copia entra logo abaixo da original, com preco e "vende por quantidade" iguais), a RECUSA DE CODIGO REPETIDO (no cadastro e na geracao, inclusive o caso que so o Pix enxerga -- "MINI-1H" x "MINI1H" --, e o negativo: salvar uma edicao sem mexer no codigo nao pode ser recusada), o ARRANJO dos botoes (a faixa embaixo do item, os cinco botoes, e o simbolo do duplicar que nao pode ser um retangulo vazio na fonte do sistema) e o AVISO DE PENDENTES citando o rotulo que esta na tela. Cada caso corre num `try` proprio: numa arvore sem o botao o caso acusa e o roteiro segue. | Ao mexer em `aPacDuplicar`/`aOpDuplicar`, em `aPacEditar`/`aPacLimpar`/`aPacSalvar`, em `fcDupCod`/`fcDupNome`/`fcCtrl`, ou ao levar o duplicar a outra aba. A regressao NAO alcanca isto: o catalogo pode PERDER UM PACOTE INTEIRO sem que uma linha do bloco gerado mude de forma -- muda o conteudo, que e o que ninguem ve num diff de 24 mil bytes. Medido na `main` de 03/09/2026: 11 das 23 verificacoes falham la, e duas delas sao defeitos que existem la (codigo repetido aceito no cadastro; o aviso de pendentes mandando apertar um botao que ja nao esta na tela). `node scripts/verificar/duplicar-itens.mjs`. |
...
- **A INTERFACE da ferramenta fica quase toda fora.** `geradores.mjs` preenche
  ...
  "Aplicar") e `duplicar-itens.mjs` um terceiro (o botao de duplicar da aba
  Agendamento por pacote) porque foi ali que os defeitos apareceram; o resto
